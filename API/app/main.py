...
from .auth import (
    ...
    create_refresh_token, is_refresh_token_valid
)
...
# ---------------------------------------
# Récupérer l'utilisateur courant (JWT + refresh)
# ---------------------------------------
def get_current_user(
    ...
    # Access token invalide / expiré → tenter refresh
    if x_refresh_token is None:
        raise HTTPException(status_code=401, detail="Invalid or expired token, no refresh token provided")

    user = db.query(User).filter(User.refresh_token == x_refresh_token).first()
    if not user or not is_refresh_token_valid(user.refresh_token_expires_at):
        raise HTTPException(status_code=401, detail="Invalid refresh token")

    # Générer un nouveau access token
    new_access_token = create_access_token({"sub": user.id})

    # Ajouter le nouveau token dans le header pour que le client puisse le récupérer
    response = JSONResponse(content={"message": "Access token refreshed"})
    response.headers["X-New-Access-Token"] = new_access_token
    # ⚠ Note: ici on retourne l'utilisateur normalement pour que la route continue
    # Dans Swagger, tu dois lire le header X-New-Access-Token pour mettre à jour ton token
    return user
...
def daily_stats(user_id: int = Query(...), day: date = Query(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    consumptions = db.query(Consumption).filter(Consumption.user_id == user_id, Consumption.date == day).all()
...
def weekly_stats(user_id: int = Query(...), start_day: date = Query(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    consumptions = db.query(Consumption).filter(Consumption.user_id == user_id, Consumption.date >= start_day, Consumption.date <= end_day).all()
    total_calories = total_protein = total_carbs = total_fat = 0
    for c in consumptions:
        total_calories += (c.food.calories_100g * c.quantity / 100)
        total_protein  += (c.food.protein_100g * c.quantity / 100)
...
@app.post("/refresh_token", tags=["POST Methods"])
def refresh_token_endpoint(token: str = Body(...), db: Session = Depends(get_db)):
...
def create_consumption(consumption: ConsumptionCreate = Body(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def change_password(user_id: int, passwords: PasswordChange, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
